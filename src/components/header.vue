      ...
      <div class="header-user-con">
        ...
        <el-dropdown @command="handleLangCommand">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item
                :command="locale.itemCode"
                v-for="locale in languageData"
                :key="`locale-${locale}`"
                >{{ locale.itemName }}</el-dropdown-item
              >
            </el-dropdown-menu>
          </template>
        </el-dropdown>
...
import { apiGetMultiLang } from '../../src/api/index'
...
const languageData = computed(() => {
  console.log(tags, "langaa");
  return tags.langArr;
});
// console.log(i18n, "i111");

const handleLangCommand = (command: string) => {
  const langList = i18n.global.messages as any;
  console.log(langList);
  setLocalStorage("locale", command);
  console.log(command, "1111");
  tags.setLangVal(command);
};
...
handleLangCommand(tags.getLangVal)
});
